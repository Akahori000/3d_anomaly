...
testn = {'lamp':'928', 'chair':'756', 'table':'616', 'car':'280', 'sofa':'508', 'rifle':'404', 'airplane':'576'} # test のとき
valn = {'lamp':'464', 'chair':'378', 'table':'308', 'car':'140', 'sofa':'254', 'rifle':'204', 'airplane':'288'} # val のとき

names = ['lamp', 'chair', 'table', 'car', 'sofa', 'rifle', 'airplane']
epoclist = [150, 200, 250, 299]


data_dir = './data/calculated_features_random_batchsize40_temp/'
    ...
    for i in range(CLASS_NUM):
        ...
        if os.path.exists(data_dir + model_type + anomaly_class + '/both_features/'):
            for j in range(4):
                ...
                print(path)
                kernel_path = path + 'kernel_pred_all/'
    ...
    for i in range(CLASS_NUM):
        ...
        if os.path.exists(data_dir + model_type + anomaly_class + '/both_features/'):
            for j in range(4):
                ...
                path = data_dir + model_type + anomaly_class + '/both_features/c_epoc_' + epoc + '_data' + valn[anomaly_class] + '/'
                print(path)
                linear_path = path + 'pred_NA_halfhalf_all/'
